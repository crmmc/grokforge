...
func TestIntegration_SeedToModelsEndpoint(t *testing.T) {
	...
	// grok-4.20: default, fast, think, expert, heavy → 5 request names
	// grok-imagine-image: default, pro → 2 request names
	// grok-imagine-image-lite: default → 1 request name
	// grok-imagine-image-edit: default → 1 request name
	// grok-imagine-video: default → 1 request name
	// Total: 10
	assert.Len(t, ids, 10, "seed should produce 10 model entries")
...
// BE-002: POST create family (auto-creates default mode) → GET /v1/models includes new model.
func TestIntegration_CreateFamilyMode_VisibleInModels(t *testing.T) {
	_, _, srv := setupModelIntegrationStack(t, false)

	// Create family — auto-creates default mode
	rec := postJSON(t, srv, "/admin/models/families",
		`{"model":"test-chat","display_name":"Test Chat","type":"chat","pool_floor":"basic","upstream_model":"grok-3","default_upstream_mode":"auto"}`)
	require.Equal(t, http.StatusCreated, rec.Code)

	var fam FamilyResponse
	require.NoError(t, json.NewDecoder(rec.Body).Decode(&fam))

	// "test-chat" should already be in /v1/models (default mode auto-created)
	ids := modelsResponseIDs(t, srv)
...
func TestIntegration_DeleteFamily_RemovedFromModels(t *testing.T) {
	...
	// Create family (auto-creates default mode)
	rec := postJSON(t, srv, "/admin/models/families",
		`{"model":"ephemeral","display_name":"Ephemeral","type":"chat","pool_floor":"basic","upstream_model":"grok-3","default_upstream_mode":"auto"}`)
	require.Equal(t, http.StatusCreated, rec.Code)
	var fam FamilyResponse
	require.NoError(t, json.NewDecoder(rec.Body).Decode(&fam))
...
func TestIntegration_TypeChangeRollback_AtomicRegistry(t *testing.T) {
	...
	// Create image family (auto-creates default mode, no upstream needed for image_ws)
	rec := postJSON(t, srv, "/admin/models/families",
		`{"model":"img-test","display_name":"Img","type":"image_ws","pool_floor":"super"}`)
	require.Equal(t, http.StatusCreated, rec.Code)
	var fam FamilyResponse
	require.NoError(t, json.NewDecoder(rec.Body).Decode(&fam))

	// Try to change type to chat — should fail because mode has no upstream
	body := `{"model":"img-test","display_name":"Img","type":"chat","pool_floor":"super","upstream_model":"grok-3"}`
...
func TestIntegration_NonDefaultModeRequestName(t *testing.T) {
	...
	// Create family (auto-creates default mode)
	rec := postJSON(t, srv, "/admin/models/families",
		`{"model":"mymodel","display_name":"My Model","type":"chat","pool_floor":"basic","upstream_model":"grok-3","default_upstream_mode":"auto"}`)
	require.Equal(t, http.StatusCreated, rec.Code)
	var fam FamilyResponse
	require.NoError(t, json.NewDecoder(rec.Body).Decode(&fam))

	// Create "fast" mode
	body := fmt.Sprintf(`{"model_id":%d,"mode":"fast","upstream_mode":"MODE_FAST"}`, fam.ID)
...
func TestIntegration_PoolFloorOverride_TokenRouting(t *testing.T) {
	...
	// Create family with pool_floor=basic (auto-creates default mode)
	rec := postJSON(t, srv, "/admin/models/families",
		`{"model":"route-test","display_name":"Route Test","type":"chat","pool_floor":"basic","upstream_model":"grok-3","default_upstream_mode":"auto"}`)
	require.Equal(t, http.StatusCreated, rec.Code)
	var fam FamilyResponse
	require.NoError(t, json.NewDecoder(rec.Body).Decode(&fam))

	// Create "heavy" mode with pool_floor_override=heavy
	body := fmt.Sprintf(`{"model_id":%d,"mode":"heavy","upstream_mode":"MODE_HEAVY","pool_floor_override":"heavy"}`, fam.ID)
	rec = postJSON(t, srv, "/admin/models/modes", body)
	require.Equal(t, http.StatusCreated, rec.Code)
...
func TestIntegration_ConflictDetection_FamilyVsDerivedName(t *testing.T) {
	...
	// Create "grok-3" family (auto-creates default mode)
	rec := postJSON(t, srv, "/admin/models/families",
		`{"model":"grok-3","display_name":"Grok 3","type":"chat","pool_floor":"basic","upstream_model":"grok-3","default_upstream_mode":"auto"}`)
	require.Equal(t, http.StatusCreated, rec.Code)
	var fam FamilyResponse
	require.NoError(t, json.NewDecoder(rec.Body).Decode(&fam))

	// Create "mini" mode → derived name "grok-3-mini"
	body := fmt.Sprintf(`{"model_id":%d,"mode":"mini","upstream_mode":"MODE_MINI"}`, fam.ID)
	rec = postJSON(t, srv, "/admin/models/modes", body)
	require.Equal(t, http.StatusCreated, rec.Code)

	// Try to create "grok-3-mini" family → 409 conflict
	rec = postJSON(t, srv, "/admin/models/families",
		`{"model":"grok-3-mini","display_name":"Grok 3 Mini","type":"chat","pool_floor":"basic","upstream_model":"grok-3","default_upstream_mode":"auto"}`)
...
func TestIntegration_ConflictDetection_ModeVsFamilyName(t *testing.T) {
	...
	// Create "grok-3-fast" family (auto-creates default mode)
	rec := postJSON(t, srv, "/admin/models/families",
		`{"model":"grok-3-fast","display_name":"Grok 3 Fast","type":"chat","pool_floor":"basic","upstream_model":"grok-3","default_upstream_mode":"auto"}`)
	require.Equal(t, http.StatusCreated, rec.Code)
...
func TestIntegration_DisabledFamily_ExcludedFromModels(t *testing.T) {
	...
	// Create enabled family (auto-creates default mode)
	rec := postJSON(t, srv, "/admin/models/families",
		`{"model":"enabled-fam","display_name":"Enabled","type":"chat","pool_floor":"basic","enabled":true,"upstream_model":"grok-3","default_upstream_mode":"auto"}`)
	require.Equal(t, http.StatusCreated, rec.Code)

	// Create disabled family (auto-creates default mode)
	rec = postJSON(t, srv, "/admin/models/families",
		`{"model":"disabled-fam","display_name":"Disabled","type":"chat","pool_floor":"basic","enabled":false,"upstream_model":"grok-3","default_upstream_mode":"auto"}`)
	require.Equal(t, http.StatusCreated, rec.Code)
...
func TestIntegration_DisabledMode_ExcludedFromModels(t *testing.T) {
	...
	// Create family (auto-creates default mode)
	rec := postJSON(t, srv, "/admin/models/families",
		`{"model":"mode-test","display_name":"Mode Test","type":"chat","pool_floor":"basic","enabled":true,"upstream_model":"grok-3","default_upstream_mode":"auto"}`)
	require.Equal(t, http.StatusCreated, rec.Code)
	var fam FamilyResponse
	require.NoError(t, json.NewDecoder(rec.Body).Decode(&fam))

	// Create disabled "fast" mode
	body := fmt.Sprintf(`{"model_id":%d,"mode":"fast","upstream_mode":"MODE_FAST","enabled":false}`, fam.ID)
	rec = postJSON(t, srv, "/admin/models/modes", body)
	require.Equal(t, http.StatusCreated, rec.Code)
...
func TestIntegration_UpdatePoolFloor_RegistryImmediate(t *testing.T) {
	...
	// Create family (auto-creates default mode)
	rec := postJSON(t, srv, "/admin/models/families",
		`{"model":"floor-test","display_name":"Floor","type":"chat","pool_floor":"basic","enabled":true,"upstream_model":"grok-3","default_upstream_mode":"auto"}`)
	require.Equal(t, http.StatusCreated, rec.Code)
	var fam FamilyResponse
	require.NoError(t, json.NewDecoder(rec.Body).Decode(&fam))
	...
	body := `{"model":"floor-test","display_name":"Floor","type":"chat","pool_floor":"super","upstream_model":"grok-3"}`
	rec = putJSON(t, srv, fmt.Sprintf("/admin/models/families/%d", fam.ID), body)
	require.Equal(t, http.StatusOK, rec.Code)
...
func TestIntegration_CreateMode_RegistryImmediate(t *testing.T) {
	...
	// Create family (auto-creates default mode)
	rec := postJSON(t, srv, "/admin/models/families",
		`{"model":"turbo-fam","display_name":"Turbo","type":"chat","pool_floor":"basic","enabled":true,"upstream_model":"grok-3","default_upstream_mode":"auto"}`)
	require.Equal(t, http.StatusCreated, rec.Code)
	var fam FamilyResponse
	require.NoError(t, json.NewDecoder(rec.Body).Decode(&fam))
	...
	// Add "turbo" mode
	body := fmt.Sprintf(`{"model_id":%d,"mode":"turbo","upstream_mode":"MODE_TURBO","enabled":true}`, fam.ID)
	rec = postJSON(t, srv, "/admin/models/modes", body)
	require.Equal(t, http.StatusCreated, rec.Code)
...
func TestIntegration_ImageModeRejectsUpstream(t *testing.T) {
	...
	// Create image_ws family (auto-creates default mode)
	rec := postJSON(t, srv, "/admin/models/families",
		`{"model":"img-reject","display_name":"Img","type":"image_ws","pool_floor":"super","enabled":true}`)
	require.Equal(t, http.StatusCreated, rec.Code)
	var fam FamilyResponse
	require.NoError(t, json.NewDecoder(rec.Body).Decode(&fam))

	// Try to create mode with upstream on image_ws family → 400
	body := fmt.Sprintf(`{"model_id":%d,"mode":"pro","upstream_mode":"MODE_FAST"}`, fam.ID)
	rec = postJSON(t, srv, "/admin/models/modes", body)
	assert.Equal(t, http.StatusBadRequest, rec.Code)
}

// BE-014: CreateMode rejects mode="default" since default is auto-created by CreateFamily.
func TestIntegration_CreateModeRejectsDefault(t *testing.T) {
	_, _, srv := setupModelIntegrationStack(t, false)

	// Create family (auto-creates default mode)
	rec := postJSON(t, srv, "/admin/models/families",
		`{"model":"first-mode","display_name":"First","type":"chat","pool_floor":"basic","enabled":true,"upstream_model":"grok-3","default_upstream_mode":"auto"}`)
	require.Equal(t, http.StatusCreated, rec.Code)
	var fam FamilyResponse
	require.NoError(t, json.NewDecoder(rec.Body).Decode(&fam))

	// Try to create mode="default" manually → 400
	body := fmt.Sprintf(`{"model_id":%d,"mode":"default","upstream_mode":"MODE_EXPERT"}`, fam.ID)
	rec = postJSON(t, srv, "/admin/models/modes", body)
	assert.Equal(t, http.StatusBadRequest, rec.Code)
}

// BE-015: Delete default mode → rejected entirely.
func TestIntegration_DeleteDefaultModeWithOthers_Rejected(t *testing.T) {
	_, _, srv := setupModelIntegrationStack(t, false)

	// Create family (auto-creates default mode)
	rec := postJSON(t, srv, "/admin/models/families",
		`{"model":"del-default","display_name":"Del","type":"chat","pool_floor":"basic","enabled":true,"upstream_model":"grok-3","default_upstream_mode":"auto"}`)
	require.Equal(t, http.StatusCreated, rec.Code)
	var fam FamilyResponse
	require.NoError(t, json.NewDecoder(rec.Body).Decode(&fam))

	// Get the auto-created default mode ID
	require.NotNil(t, fam.DefaultModeID)
	defaultModeID := *fam.DefaultModeID

	// Create expert mode
	body := fmt.Sprintf(`{"model_id":%d,"mode":"expert","upstream_mode":"MODE_EXPERT","enabled":true}`, fam.ID)
	rec = postJSON(t, srv, "/admin/models/modes", body)
	require.Equal(t, http.StatusCreated, rec.Code)

	// Try to delete default mode → 400
	rec = doDelete(t, srv, fmt.Sprintf("/admin/models/modes/%d", defaultModeID))
	assert.Equal(t, http.StatusBadRequest, rec.Code)
}
