...
func testModeSpecs() []modelconfig.ModeSpec {
	return []modelconfig.ModeSpec{
		{
			ID:            "auto",
			UpstreamName:  "auto",
			WindowSeconds: 7200,
			DefaultQuota:  map[string]int{"basic": 0, "super": 50, "heavy": 150},
		},
		{
			ID:            "fast",
			UpstreamName:  "fast",
			WindowSeconds: 86400,
			DefaultQuota:  map[string]int{"basic": 30, "super": 140, "heavy": 400},
		},
	}
}
...
func TestManager_ClearModeQuota(t *testing.T) {
	...
	assert.Equal(t, 0, mgr.GetInflight(1), "ClearModeQuota should release inflight")
}
