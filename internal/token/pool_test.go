...
func TestPool_Select_HighQuotaFirst(t *testing.T) {
	...
	pool.Add(&store.Token{ID: 1, Token: "t1", Status: string(StatusActive), Quotas: store.IntMap{"auto": 50}})
	pool.Add(&store.Token{ID: 2, Token: "t2", Status: string(StatusActive), Quotas: store.IntMap{"auto": 80}})
	pool.Add(&store.Token{ID: 3, Token: "t3", Status: string(StatusActive), Quotas: store.IntMap{"auto": 30}})

	selected := pool.Select(AlgoHighQuotaFirst, "auto")
...
func TestPool_Select_Random(t *testing.T) {
	...
	pool.Add(&store.Token{ID: 1, Token: "t1", Status: string(StatusActive), Quotas: store.IntMap{"auto": 50}})
	pool.Add(&store.Token{ID: 2, Token: "t2", Status: string(StatusActive), Quotas: store.IntMap{"auto": 50}})
	pool.Add(&store.Token{ID: 3, Token: "t3", Status: string(StatusActive), Quotas: store.IntMap{"auto": 50}})

	// Run multiple selections and check we don't always get the same one
	selections := make(map[uint]int)
	for i := 0; i < 100; i++ {
		selected := pool.Select(AlgoRandom, "auto")
		require.NotNil(t, selected)
...
func TestPool_Select_RoundRobin(t *testing.T) {
	...
	for i := 0; i < 3; i++ {
		selected := pool.Select(AlgoRoundRobin, "auto")
		require.NotNil(t, selected)
...
func TestPool_Select_PriorityTiers(t *testing.T) {
	...
	pool.Add(&store.Token{ID: 1, Token: "t1", Status: string(StatusActive), Quotas: store.IntMap{"auto": 50}, Priority: 3})
	pool.Add(&store.Token{ID: 2, Token: "t2", Status: string(StatusActive), Quotas: store.IntMap{"auto": 80}, Priority: 3})
	pool.Add(&store.Token{ID: 3, Token: "t3", Status: string(StatusActive), Quotas: store.IntMap{"auto": 100}, Priority: 1})
...
func TestPool_Select_PriorityTiers_FallThrough(t *testing.T) {
	...
	// pri=3 has only disabled tokens, pri=0 has active
	pool.Add(&store.Token{ID: 1, Token: "t1", Status: string(StatusDisabled), Quotas: store.IntMap{"auto": 100}, Priority: 3})
	pool.Add(&store.Token{ID: 2, Token: "t2", Status: string(StatusActive), Quotas: store.IntMap{"auto": 50}, Priority: 0})
...
func TestPool_Select_EmptyPool(t *testing.T) {
	...
	for _, algo := range []string{AlgoHighQuotaFirst, AlgoRandom, AlgoRoundRobin} {
		selected := pool.Select(algo, "auto")
...
func TestPool_Select_IgnoresNonActiveTokens(t *testing.T) {
	...
	pool.Add(&store.Token{ID: 1, Token: "t1", Status: string(StatusExpired), Quotas: store.IntMap{"auto": 100}})
	pool.Add(&store.Token{ID: 2, Token: "t2", Status: string(StatusDisabled), Quotas: store.IntMap{"auto": 100}})
	pool.Add(&store.Token{ID: 3, Token: "t3", Status: string(StatusActive), Quotas: store.IntMap{"auto": 50}})
...
func TestPool_Count(t *testing.T) {
	...
	pool.Add(&store.Token{ID: 1, Token: "t1", Status: string(StatusActive), Quotas: store.IntMap{"auto": 100}})
	pool.Add(&store.Token{ID: 2, Token: "t2", Status: string(StatusActive), Quotas: store.IntMap{"auto": 80}})
	pool.Add(&store.Token{ID: 3, Token: "t3", Status: string(StatusExpired), Quotas: store.IntMap{"auto": 60}})
	pool.Add(&store.Token{ID: 4, Token: "t4", Status: string(StatusDisabled), Quotas: store.IntMap{"auto": 40}})
	...
	assert.Equal(t, 1, expired, "should count 1 expired token")
}

func TestPool_AddAndRemove(t *testing.T) {
	pool := NewTokenPool(PoolBasic)

	token := &store.Token{ID: 1, Token: "t1", Status: string(StatusActive), Quotas: store.IntMap{"auto": 100}}
	pool.Add(token)

	active, _, _ := pool.Count()
