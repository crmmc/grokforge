package mediautil

import (
	"bytes"
	"context"
	"image"
	"image/png"
	"net/http"
	"net/http/httptest"
	"strings"
	"testing"

	"github.com/crmmc/grokforge/internal/upstream"
)

func TestParseMultimodalContent_String(t *testing.T) {
	content := "Hello, world!"
	blocks, err := ParseMultimodalContent(content)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if len(blocks) != 1 {
		t.Fatalf("expected 1 block, got %d", len(blocks))
	}
	if blocks[0].Type != "text" {
		t.Errorf("expected type text, got %s", blocks[0].Type)
	}
	if blocks[0].Text != content {
		t.Errorf("expected text %q, got %q", content, blocks[0].Text)
	}
}

func TestParseMultimodalContent_Array(t *testing.T) {
	content := []any{
		map[string]any{"type": "text", "text": "Check this image:"},
		map[string]any{
			"type": "image_url",
			"image_url": map[string]any{
				"url":    "data:image/png;base64,iVBORw0KGgo=",
				"detail": "high",
			},
		},
	}

	blocks, err := ParseMultimodalContent(content)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if len(blocks) != 2 {
		t.Fatalf("expected 2 blocks, got %d", len(blocks))
	}

	if blocks[0].Type != "text" {
		t.Errorf("block 0: expected type text, got %s", blocks[0].Type)
	}
	if blocks[0].Text != "Check this image:" {
		t.Errorf("block 0: unexpected text %q", blocks[0].Text)
	}

	if blocks[1].Type != "image_url" {
		t.Errorf("block 1: expected type image_url, got %s", blocks[1].Type)
	}
	if blocks[1].ImageURL == nil {
		t.Fatal("block 1: ImageURL is nil")
	}
	if !strings.HasPrefix(blocks[1].ImageURL.URL, "data:image/png") {
		t.Errorf("block 1: unexpected URL %q", blocks[1].ImageURL.URL)
	}
	if blocks[1].ImageURL.Detail != "high" {
		t.Errorf("block 1: expected detail high, got %s", blocks[1].ImageURL.Detail)
	}
}

func TestParseMultimodalContent_TypedBlocks(t *testing.T) {
	content := []upstream.ContentBlock{{Type: "text", Text: "typed"}}

	blocks, err := ParseMultimodalContent(content)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if len(blocks) != 1 || blocks[0].Text != "typed" {
		t.Fatalf("unexpected blocks: %#v", blocks)
	}
}

func TestParseMultimodalContent_MapSlice(t *testing.T) {
	content := []map[string]any{{"type": "text", "text": "mapped"}}

	blocks, err := ParseMultimodalContent(content)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if len(blocks) != 1 || blocks[0].Text != "mapped" {
		t.Fatalf("unexpected blocks: %#v", blocks)
	}
}

func TestParseMultimodalContent_InvalidType(t *testing.T) {
	_, err := ParseMultimodalContent(12345)
	if err == nil {
		t.Error("expected error for invalid content type")
	}
}

func TestProcessImageURL_DataURI(t *testing.T) {
	dataURI := "data:image/jpeg;base64,/9j/4AAQSkZJRg=="
	result, err := processImageURL(context.Background(), dataURI)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if result != dataURI {
		t.Errorf("data URI should pass through unchanged")
	}
}

func TestProcessImageURL_HTTPDownload(t *testing.T) {
	jpegData := []byte{
		,
	}

	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "image/jpeg")
		w.Write(jpegData)
	}))
	defer server.Close()

	_, err := processImageURL(context.Background(), server.URL)
	if err == nil {
		t.Fatal("expected error for private/internal IP, got nil")
	}
	if !strings.Contains(err.Error(), "blocked") {
		t.Fatalf("unexpected error: %v", err)
	}
}

func TestProcessContent(t *testing.T) {
	blocks := []upstream.ContentBlock{
		{Type: "text", Text: "Hello "},
		{Type: "text", Text: "World"},
		{Type: "image_url", ImageURL: &upstream.ImageURLBlock{
			URL: "data:image/png;base64,abc123",
		}},
	}

	result, err := ProcessContent(context.Background(), blocks)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}

	if result.Text != "Hello World" {
		t.Errorf("expected combined text 'Hello World', got %q", result.Text)
	}
	if len(result.Images) != 1 {
		t.Fatalf("expected 1 image, got %d", len(result.Images))
	}
	if result.Images[0] != "data:image/png;base64,abc123" {
		t.Errorf("unexpected image URL: %s", result.Images[0])
	}
}

func TestProcessImageData_PreservesPNG(t *testing.T) {
	dataURI, err := processImageData(createTestPNG(), "image/png")
	if err != nil {
		t.Fatalf("processImageData() error = %v", err)
	}
	if !strings.HasPrefix(dataURI, "data:image/png;base64,") {
		t.Fatalf("processImageData() = %q, want png data URI", dataURI)
	}
}

func TestResizeImageMaxDim(t *testing.T) {
	img := image.NewRGBA(image.Rect(0, 0, 5000, 3000))

	resized := resizeImageMaxDim(img, 4096)
	bounds := resized.Bounds()

	if bounds.Dx() != 4096 {
		t.Errorf("expected width 4096, got %d", bounds.Dx())
	}
	expectedH := 3000 * 4096 / 5000
	if bounds.Dy() != expectedH {
		t.Errorf("expected height %d, got %d", expectedH, bounds.Dy())
	}
}

func TestResizeImageMaxDim_NoResize(t *testing.T) {
	img := image.NewRGBA(image.Rect(0, 0, 2000, 1000))

	resized := resizeImageMaxDim(img, 4096)

	if resized != img {
		t.Error("image under limit should not be resized")
	}
}

func createTestPNG() []byte {
	img := image.NewRGBA(image.Rect(0, 0, 100, 100))
	var buf bytes.Buffer
	_ = png.Encode(&buf, img)
	return buf.Bytes()
}
